...
class ComentarioNegocioController extends Controller
{
    ...
    public function comentarNegocioPerfilPublicoNewAction($slug_negocio,$id,Request $request){
        ...
        if(is_object($user)){
            ...
            $form = $this->createForm(new ComentarioNegocioType(),$comentarioNegocio,array(
                'action' => $this->generateUrl('negocio_perfil_publico_comentar_new',array('slug_negocio'=>$slug_negocio,'id'=>$id)),
                'method' => 'POST',
            ));
            //$form = $this->createForm(new ComentarioProveedorType(), $comentarioProveedor);
            $form->handleRequest($request);
            if ($form->isSubmitted()) {
                if($form->isValid()){
                    ...
                    $request->getSession()->getFlashBag()->add('success', 'Gracias por tu comentario !');
                    //return $this->redirectToRoute('proveedor_detail',array('slug_site'=>$slug_site,'slug_proveedor'=>$slug_proveedor));
                    $url = $this->generateUrl('profile_public',array('id'=>$id));
                    $response = new JsonResponse();
                    $response->setData(array(
                        'success' => $url
                    ));
    ...
    public function comentarNegocioPerfilPublicoEditAction($slug_negocio,$id,Request $request){
        ...
        if(is_object($user)){
            ...
            $form = $this->createForm(new ComentarioNegocioType(),$comentarioNegocio,array(
                'action' => $this->generateUrl('negocio_perfil_publico_comentar_edit',array('slug_negocio'=>$slug_negocio,'id'=>$id)),
                'method' => 'POST',
            ));
            //$form = $this->createForm(new ComentarioProveedorType(), $comentarioProveedor);
            $form->handleRequest($request);
